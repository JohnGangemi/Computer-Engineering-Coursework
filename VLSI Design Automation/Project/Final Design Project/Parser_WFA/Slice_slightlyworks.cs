        ...
        public Boolean fillSlice(int partition_width, int partition_length, int slice_width, int slice_length, int partition_number)
        {
            ...
            for (int i = 0; i < slice_length; i++)
            {
                for (int j = slice_width - 1; j >= 0; j--)
                {
                    ...
                    else if (slice_matrix[i, j] == 0 && length_remaining[i] >= partition_length && width_remaining[j] >= partition_width && !done) //Original
                    {
                        for (int g = i; g < partition_length; g++)
                        {
                        ...
                        for (int l = i; l < partition_length; l++)
                        {
                            length_remaining[l] -= partition_length;
                        }
                        for (int w = j; w > j - partition_width; w--)
                        {
                            width_remaining[w] -= partition_width;
                        }
                        done = true;
                    }
